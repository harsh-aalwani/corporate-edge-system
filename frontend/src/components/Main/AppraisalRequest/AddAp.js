...
const AddEmployeeAppraisal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/employee-appraisal/create", formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      enqueueSnackbar("Appraisal Request Sent Successfully!", { variant: "success" });
      setFormData({
        appraisalTitle: "",
        appraisalDescription: "",
        departmentId: "",
      ...
      enqueueSnackbar(error.response?.data?.error || "Failed to submit appraisal request", { variant: "error" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="container mt-5 px-5">
      <div className="card p-4 page-box">
        <h4 className="mb-3 text-center">Add Employee Appraisal</h4>
        <form className="custom-form" onSubmit={handleSubmit} encType="multipart/form-data">
          <div className="form-group full-width">
            <label className="form-label">Title:</label>
            <input type="text" name="appraisalTitle" className="form-control" value={formData.appraisalTitle} onChange={handleChange} />
          </div>
          <div className="form-group full-width">
            <label className="form-label">Description:</label>
            <textarea name="appraisalDescription" className="form-control" value={formData.appraisalDescription} onChange={handleChange} rows="4" />
          </div>
