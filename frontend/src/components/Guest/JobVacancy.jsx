...
const JobVacancyForm = () => {
  ...
  const hasErrors =
    ...
    !!languagesError ||
    !!specializationError ||
    Object.values(nameError).some((err) => err);
    ...
    const fetchPositions = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/announcements/jobdata"
        );
    ...
    setFormData((prev) => {
      ...
      if (nameFields.includes(name)) {
        ...
        if (!nameRegex.test(value)) {
          ...
          setNameError((prevError) => ({ ...prevError, [name]: "" }));
        }
        updatedForm[name] = value;
        return updatedForm;
      }
      if (name === "languagesKnown") {
        const langRegex = /^[A-Za-z\s,().\/-]*$/;;
      ...
      if (name === "phone") {
        ...
        if (!phoneRegex.test(value)) {
          setPhoneError(
            "Phone number must contain digits only and be at most 10 digits."
          );
        } else {
          setPhoneError("");
        }
      ...
      if (name === "specialization") {
        ...
        if (!specRegex.test(value)) {
          setSpecializationError(
            "Only alphabetic characters and spaces are allowed for Specialization."
          );
        } else {
          setSpecializationError("");
        }
      ...
      if (name === "yearsOfExperience") {
        ...
        if (enteredYears > totalYears) {
          setErrorForTY(
            "Years of Experience cannot be greater than Total Years of Experience."
          );
        } else {
      ...
      if (name === "totalYearsOfExperience") {
        ...
        if (totalYears < enteredYears) {
          // ✅ Fixed condition: It should be `<` not `<=`
          setErrorForTY(
            "Total Years of Experience must be greater than or equal to Years of Experience."
          );
        } else {
          setErrorForTY(""); // Clear error if valid
        }
    ...
    try {
      ...
      enqueueSnackbar(
        error.response?.data?.message ||
          "Error creating candidate. Please try again.",
        { variant: "error" }
      );
    } finally {
      setIsLoading(false);
        ...
        {!isLoading && !isSuccess && (
          <>
            <h3 className="mb-3 text-center" style={{ fontWeight: "bold" }}>
              {" "}
              <FaFileWaveform class="mb-2" /> Job Application Form
            </h3>
            ...
            >
              <div className="row">
                ...
                <div className="col-md-6 mb-3">
                  <label className="form-label">Picture: [Only .png, .jpeg, .jpg, .gif, .webp]</label>
                  <input
                    type="file"
                  ...
                  <div class="row">
                    <div className="col-md-6 mb-3">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const selectedAnnouncement = positions.find(
                            (job) => job.announcementId === e.target.value
                          );
                          setFormData({
                            ...formData,
                            announcementId: e.target.value,
                            position: selectedAnnouncement
                              ? selectedAnnouncement.position
                              : "",
                            departmentId: selectedAnnouncement
                              ? selectedAnnouncement.departmentId
                              : "", // ✅ Store departmentId for server
                            department: selectedAnnouncement
                              ? selectedAnnouncement.departmentName
                              : "", // ✅ Show departmentName to user
                          });
                        }}
                        required
                      >
                        ...
                        {positions.map((job) => (
                          <option
                            key={job.announcementId}
                            value={job.announcementId}
                          >
                            {job.position}
                          </option>
                        ))}
                ...
                {TotalYearError && (
                  <p className="text-danger">{TotalYearError}</p>
                )}
        ...
        {isSuccess && (
          <div className="text-center my-5 p-5">
            <h3 className="text-success fw-bold">
              Successfully Added as Candidate
            </h3>
