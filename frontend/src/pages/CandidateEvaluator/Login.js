...
const Login = () => {
  ...
  const handleLoginSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/users/evaluatorsLogin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userEmail: trimmedEmail, userPassword: trimmedPassword }),
        credentials: 'include',
      });
      ...
      if (response.ok) {
        ...
        setUserRoleCookie(data.encryptedRole);
        navigate('/EvaluatorDashboard'); // Redirect after login
      } else {
